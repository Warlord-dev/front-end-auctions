import React from 'react'
import styles from './page34.module.scss'
const Page34 = () => {
  return (
    <div className={styles.wrapper}>
      <p className={styles.text1}>The Art in Post Selection.</p>
      <p className={styles.text2}>
        Right now I’m making a giant remix of everything that has come before and everything that is 
        to come after. Right now I’m playing with these time crystals. I’m in this liminal place 
        between where I’ve been and where I’m going, and I’m putting everything on the line. It might 
        feel a little uncomfortable sometimes, even a little scary. But the truth is, this doesn’t 
        really bother me too much. In fact, I’m kind of addicted to it. I’m sticking with it. This is 
        an all in game. A half-hearted spirit has no power. Because I know exactly what direction I’m 
        headed and who’s chosen to come along for the ride. It’s a ride filled with intense passion. 
        And something good is happening.
        <br />
        <br />
        As we cultivate this new phase of matter it’s easy to get pulled into a gravity well of 
        collapsing paridigms and tempt drifting past the event horizon. Because being content with how 
        things are is how we slip into an area of low mass, there’s nothing to bump into or push off from. 
        It’s a grey zone of rebalancing. There’s no clash to stir up catalysts of new innovation and new 
        hope. Again and again for many this slow fade into inertia just keeps repeating. 
        <br />
        <br />
        So where does a real spark come from? Where is the magic beans moment that against all odds 
        carries us up and out. It certainly doesn’t come from brute force or persistant logic. Instead, 
        the champion moment can be instigated through our selective ability to hit the magic reset button.
        Because the past is never where you think that you left it. This is a tool that allows us to bypass 
        irrational fantasies and skip right to the good stuff. The post select. We can shape our desires and
        realities even as it they shape us. This isn’t simply a matter of delicate interpretation. It’s how 
        the complete spread from our neurons to the encompassing universe work. Backwards causation gives a
        broader perspective. We zoom out in order to zoom in. 
        <br />
        <br />
        Fortunately, decentralised tech in commerce gives us these kind of super post selecting powers, where 
        we can build around a core itterable loop of economic activity that creates wealth in the core of 
        the entire mechanism, gaining time, space and greater tranformational ability as the friction points 
        of the system are radically reduced and our ability to communicate, transact, trade, buy, sell, wear 
        generously compounds on the fundamental value collectively generated. 
        <br />
        <br />
        Decentralised Commerce is not just about moving goods from one point to another. It’s much more than 
        that. It fosters and encourages fundamental breakthroughs of economic, creative and financial power 
        so that we can proactively shape our present, no longer constrained but rather amplified by the past. 
        For there are many kinds of magic, after all. 
      </p>
      <p className={styles.text12}>- Emma-Jane MacKinnon-Lee</p>
      <img src="./magazine/4/images/0304_background.jpg" className={styles.image1} />
      <img src="./magazine/4/images/0304_image1.png" className={styles.image3} />

      <div className={styles.image2}></div>
      <p className={styles.text3}>contributors</p>
      <p className={styles.text4}>
        Emma-Jane MacKinnon-Lee
        <br />
        <br />
        Isramae Visales <br />
        <br />
        Peezy R<br />
        <br />
        Victor
        <br />
        <br />
        Haruto Inoue
        <br />
        <br />
        Cameron Sato
        <br />
        <br />
        {/* Miko Matsumura */}
      </p>
      <p className={styles.text5}>
        Stan McLygin
        <br />
        <br />
        The Global Designer Network <br />
        <br />
        (GDN)
        <br />
        <br />
        Glitch Faced King
        <br />
        <br />
        James Lin
        <br />
        <br />
        {/* Eric Song 
        <br />
        <br /> */}
        Aditya
      </p>
      <p className={styles.text6}>
        Ana Rivera
        <br />
        <br />
        Rafi 0x
        <br />
        <br />
        Thakane/Takii
        <br />
        <br />
        Fabian Vogelsteller
        <br />
        <br />
        Donald Iljazi
        <br />
        <br />
        xlvrbk
      </p>
      <p className={styles.text7}>
        OurZora 
        <br />
        <br />
        Web3 Fashion Week
        <br />
        <br />
        g.mirror.xyz
        <br />
        <br />
        Strawberry Sith <br />
        <br />
        Kyle Tut
        <br />
        <br />
        Alex Adelman
      </p>
      <p className={styles.text8}>
        Charbakd
        <br />
        <br />
        Jaymes.eth 
        <br />
        <br />
        (Pirate Ape Yacht Club)
        <br />
        <br />
        Heapons
        <br />
        <br />
        BasedUser
        <br />
        <br />
        Witek Radomski
      </p>
      <p className={styles.text9}>
        Crypto Deevo
        <br />
        <br />
        Blanc de Blanc
        <br />
        <br />
        The Black Lab Studio
        <br />
        <br />
        Unhueman
        <br />
        <br />
        Xenotech
        <br />
        <br />
        DOSC
      </p>
      <p className={styles.text10}>
        Coleman Maher
        <br />
        <br />
        Gabby Dizon
        <br />
        <br />
        Jiho
        <br />
        <br />
        Love Tomorrow
        <br />
        <br />
        Maslennikov
        <br />
        <br />
        3DNINJAH
      </p>
      <p className={styles.text11}>
        Paul Leath 
        <br />
        <br />
        Christopher Leath
        <br />
        <br />
        Arturo Garce
        <br />
        <br />
        Glitched Face King
        <br />
        <br />
        Global Designer Network
      </p>
    </div>
  )
}

export default Page34
