import React from 'react'
import Link from 'next/link'
import styles from './page3334.module.scss'

const Page3334 = () => {
  return (
    <div className={styles.wrapper}>
      <div className={styles.rect1}></div>
      <div className={styles.rect2}></div>
      <div className={styles.rect3}></div>
      <div className={styles.rect4}></div>
      <div className={styles.rect5}></div>
      <div className={styles.rect6}></div>
      <div className={styles.rect7}></div>
      <div className={styles.rect8}></div>
      <div className={styles.rect9}></div>
      <div className={styles.rect10}></div>
      
      <div className={styles.line1}></div>
      <div className={styles.line2}></div>
      <div className={styles.line3}></div>
      <div className={styles.line4}></div>
      <div className={styles.line5}></div>
      <div className={styles.line6}></div>
      <div className={styles.line7}></div>
      <div className={styles.line8}></div>
      <div className={styles.line9}></div>
      <div className={styles.line10}></div>
      <div className={styles.line11}></div>
      <div className={styles.line12}></div>
      <div className={styles.line13}></div>
      <div className={styles.line14}></div>
      <div className={styles.line15}></div>

      <Link href='https://makersplace.com/nottodaynonono/perfect-breakfast-1-of-1-78082/'>
        <a target='_blank'>
          <img src='./magazine/5/images/3334_image1.png' className={styles.image1} />
        </a>
      </Link>
      <Link href='https://foundation.app/@heartfulrosee/demon-girl-80277'>
        <a target='_blank'>
          <img src='./magazine/5/images/3334_image2.png' className={styles.image2} />
        </a>
      </Link>
      <Link href='https://knownorigin.io/gallery/1296000-fuck-this-shit'>
        <a target='_blank'>
          <img src='./magazine/5/images/3334_image3.png' className={styles.image3} />
        </a>
      </Link>
      <Link href='https://www.hicetnunc.xyz/objkt/260509'>
        <a target='_blank'>
          <img src='./magazine/5/images/3334_image4.gif' className={styles.image4} />
        </a>
      </Link>
      <Link href='https://makersplace.com/nottodaynonono/perfect-breakfast-1-of-1-78082/'>
        <a target='_blank'>
          <img src='./magazine/5/images/3334_image5.png' className={styles.image5} />
        </a>
      </Link>

      <Link href='https://www.hicetnunc.xyz/objkt/260453'>
        <a target='_blank'>
          <video autoPlay muted loop className={styles.video1}>
            <source src='./magazine/5/videos/3334_video1.mp4' type='video/mp4' />
          </video>
        </a>
      </Link>

      <div className={styles.text1}>
        Crypto Art         
      </div>
      <div className={styles.text2}>
        Perfect Breakfast
      </div>
      <div className={styles.text3}>
        Perfect Breakfast
      </div>
      <div className={styles.text4}>
        Perfect Breakfast
      </div>
      <div className={styles.text5}>
        2 Sep 2021
      </div>
      <div className={styles.text6}>
        PETER FRANCE
      </div>
      <div className={styles.text7}>
        rose <span>Demon Girl</span>
      </div>
      <div className={styles.text8}>
        2 Sep 2021
      </div>
      <div className={styles.text9}>
        MAKERS
      </div>
      <div className={styles.text10}>
        MAKERS
      </div>
      <div className={styles.text11}>
        MAKERS
      </div>
      <div className={styles.text12}>
        MAKERS
      </div>
      <div className={styles.text13}>
        Margaret
      </div>
      <div className={styles.text14}>
        pototo
      </div>
      <div className={styles.text15}>
        pototo
      </div>
      <div className={styles.text16}>
        pototo
      </div>
      <div className={styles.text17}>
      ON-CHAIN. ART. NFTS. <span>CRYPTO.</span>
      </div>
      <div className={styles.text18}>
      ON-CHAIN. ART. NFTS. <span>CRYPTO.</span>
      </div>
      <div className={styles.text19}>
        3 Sep 2021
      </div>
      <div className={styles.text20}>
        3 Sep 2021
      </div>
      <div className={styles.text21}>
        Kiefmonster 
      </div>
      <div className={styles.text22}>
        Fuck This Shit
      </div>
      <div className={styles.text23}>
        Fractastic01
      </div>
      <div className={styles.text24}>
        ojosyespirales
      </div>
      <div className={styles.text25}>
        Minted on HEN
      </div>
      <div className={styles.text26}>
        MIXED FEELINGS
      </div>
      <div className={styles.text27}>
        Sept 3rd 2021
      </div>
      <div className={styles.text28}>
        arclight
      </div>

      <img src='./magazine/4/images/2930_image2.png' className={styles.image6} />

      <img src='./magazine/4/images/3334_image2.png' className={styles.image7} />
      <img src='./magazine/4/images/3334_image2.png' className={styles.image8} />
    </div>
  )
}

export default Page3334
