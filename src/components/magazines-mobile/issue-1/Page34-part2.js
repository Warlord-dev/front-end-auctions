import React from 'react'
import styles from './page34-part2.module.scss'

const Page34Part2 = () => {
  return (
    <div className={styles.wrapper}>
    
      <img className={styles.backgroundImage} src='./magazine/1/images/ezgif-frame-038_auto_x2 1.png' />
      <h2>
        contributors
      </h2>
      <h3 className={styles.content2}>
        Emma-Jane MacKinnon-Lee <br />
        Isramae Visales  <br />
        Peezy R <br />
        Victor Wiebe <br />
        Valentina Gomez  <br />
        Haruto <br />
        Cameron (Ares) <br />
      </h3>
      <h3 className={styles.content3}>
        Stan McLygin <br />
        Lad <br />
        Jason Kende <br />
        Jason Howard <br />
        John Jacinto <br />
        Glitch Faced King <br />
        James Lin <br />

      </h3>
      <h3 className={styles.content4}>
        Robotto Studio <br />
        Rudeolph <br />
        Blade Runway <br />
        Chaimholtjer <br />
        Kevin Owocki <br />
        Gemma Asedilla <br />

      </h3>
      <h3 className={styles.content5}>
        HVDSON <br />
        Jenil Thakker <br />
        Ravi Singh <br />
        Lorena Bello <br />
        Rafi 0x <br />
        Yu <br />

      </h3>
      <h3 className={styles.content6}>
        Xenotech <br />
        Pixls <br />
        Harrison Kugler <br />
        Cleora <br />
        HYPERREAALIYAH <br />
      </h3>
      <h3 className={styles.content7}>
        Aditya <br />
        Charbakd <br />
        Villa Karolina <br />
        Domingo Bette Encour <br />
        Future Creators Club <br />
        Lawrence <br />

      </h3>
      <h3 className={styles.content8}>
        Erick Stow <br />
        Matthew Chaim <br />
        Brian Benni <br />
        Gregory Gordon <br />
        Tenshi Girl <br />
        FlowersWithPowers <br />
      </h3>
      <h3 className={styles.content9}>
        Micha Almeda <br />
        Stephy Fung <br />
        Miriam McGovern <br />
        EzinCrypto <br />
        Rizzle <br />
        Annissa <br />
      </h3>
    </div>
  )
}

export default Page34Part2