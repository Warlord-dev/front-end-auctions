...
const ProductDescription = ({ clothesId, currentCollections, currentMarketplaceOffers, activeTab, setActiveTab }) => {
  const garment = useSelector(getGarmentsById(clothesId));
  const tokenUri = useMemo(() => {
    if (activeTab === 0) {
      return garment.tokenUri;
    } if (activeTab === 1) {
      const t_semiRare = currentCollections.find((collection) => collection.rarity === SEMI_RARE_RARITY);
  ...
  const currentCounts = useMemo(
    () => {
      const t_semiRare_offer = currentMarketplaceOffers.find((offer) => offer.garmentCollection.rarity === SEMI_RARE_RARITY);
      const t_common_offer = currentMarketplaceOffers.find((offer) => offer.garmentCollection.rarity === COMMON_RARITY);
      const t_semiRare = t_semiRare_offer ? currentCollections.find((collection) => collection.id === t_semiRare_offer.id) : null;
      const t_common = t_common_offer ? currentCollections.find((collection) => collection.id === t_common_offer.id) : null;

      return [
        { total: 1, sold: !garment.resulted ? 0 : 1 },
        {
          total: t_semiRare ? t_semiRare.garments.length : 0, collectionId: t_semiRare ? t_semiRare.id : null, basePrice: t_semiRare_offer ? t_semiRare_offer.primarySalePrice : '0', sold: t_semiRare_offer ? parseInt(t_semiRare_offer.amountSold, 10) : 0,
        },
        {
          total: t_common ? t_common.garments.length : 0, collectionId: t_common ? t_common.id : null, basePrice: t_common_offer ? t_common_offer.primarySalePrice : '0', sold: t_common_offer ? parseInt(t_common_offer.amountSold, 10) : 0,
        },
      ];
    },
  ...
  const currentClothesInfo = {
    ...
    description:
      tokenInfo && tokenInfo.description
        ...
        : `ID: ${clothesId}`,
    estimateApy: 127,
    youReceive: receive?.receive ? receive.receive : 'Graphic, FBX, Material Component',
