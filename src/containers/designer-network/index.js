...
const DesignerList = () => {
  ...
  const [designerCircles, setDesignerCircles] = useState(Array(designers.size).fill().map(item => Object.assign({}, designerCircle)))
  const canvasRef = useRef()
  const raf = useRef()

  useEffect(() => {
    var bubbleImage = new Image()
    var photoImages = Array(designers.size).fill().map(item => new Image())
